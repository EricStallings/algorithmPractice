...
const subsetSum = (array, target) => {
  const sortedSum = array.sort((a, b) => a-b)

  let i = 0;
  let pointer1 = i;
  let pointer2 = sortedSum.length-1;

  while(pointer1 < pointer2){
    let potentialSum = sortedSum[pointer1] + sortedSum[pointer2];
    // if (pointer1 >= pointer2) break;
    if (potentialSum === target) return true;
    if (potentialSum > target) pointer2 -= 1;
    if (potentialSum < target) pointer1 += 1;
  }
...
console.log(subsetSum([10, 15, 3, 7], 17)) //=> true
console.log(subsetSum([100, -1, 26, 7, 12, 98, 2], 101)) // false; 
