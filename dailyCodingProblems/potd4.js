...
function missingInt(array){
  ...
  for (let i = 0; i < array.length; i++){
    ...
    if (array[i] < 0 || array[i] === array[i+1] || array[i]+1 === array[i+1]){
      console.log(`Hit IF statement. array[i] is: `, array[i])
      continue;
    } else {
      return array[i] + 1;
  }
...
// console.log(missingInt([3, 4, -1, 1])) //=> 2
// console.log(missingInt([1, 2, 0])) //=> 3
// console.log(missingInt([0, 1, 2, 3, 4, 4, 6,8,7])) //=> 5
...
function firstMissingPositiveInteger(nums) {
  ...
  console.log(`Before partition, nums is: `, nums)
  const k = partition(nums, 0, nums.length - 1, 0) + 1;
  // console.log( `After PARTITION, k is: `, k)
  console.log(`AFTER partition, nums is: `, nums)

  let result = k;
  for (let i = 0; i < k; i++) {
    const indexValue = Math.abs(nums[i]);
    // console.log(`Line 91, indexValue is: `, indexValue)
    if (indexValue <= k) {
      // turn negative
  ...
  // console.log(`AFter turning negative, array is: `, nums)
...
function partition(nums, left, right, pivotValue) {
  console.log(`In PARTITION. Array is: ${nums}`)
  while (left <= right) {
    console.log(`WHILE loop 127. ${left} < = ${right}`)
    while (nums[left] > pivotValue) {
      console.log(`${nums[left]} is greater than ${pivotValue}. Incrementing LEFT...`)
      left++;
    }
    while (nums[right] < pivotValue) {
      console.log(`${nums[right]} is less than ${pivotValue}. Decrementing RIGHT...`)
      right--;
    }
    if (left <= right) {
...
function swap(arr, i, j) {
  console.log(`Arr is ${arr}. Swapping ${arr[i]} with ${arr[j]}`)
  if (i !== j) {
    const temp = arr[i];
    arr[i] = arr[j];
    arr[j] = temp;
  }
  console.log(`Swapped! Array is now ${arr}`)
}

console.log(firstMissingPositiveInteger([-1, -10, 1, 2, 4, 5, 6])) //3